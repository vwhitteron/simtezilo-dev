...
type App struct {
	...
	// Chassis + road-texture haptics: generated by app/haptics, which owns the
	// per-tick scratch and filter state. App delegates to it (see generateForceHaptics).
	chassisGen      *haptics.Generator
	transmissionGen *haptics.TransmissionGenerator
	engineGen       *haptics.EngineGenerator
...
func (a *App) initializeComponents(opts Options) error {
	...
	a.chassisGen = haptics.NewGenerator(a.config, a.synth, &a.kinematics, a.log)
	a.transmissionGen = haptics.NewTransmissionGenerator(
		a.config, a.synth, &a.kinematics, a.telemetrySource, a.log)
	a.engineGen = haptics.NewEngineGenerator(a.config, a.synth, a.telemetrySource, a.log)
...
func (a *App) gearHasChanged() bool {
	return a.transmissionGen.GearHasChanged()
}
