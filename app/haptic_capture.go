...
func CaptureHaptics(opts HapticCaptureOptions) (*HapticCapture, error) {
	if opts.Source == "" {
		return nil, fmt.Errorf("a replay Source is required")
	}

	if !opts.Engine && !opts.Chassis {
		return nil, fmt.Errorf("enable at least one of Engine or Chassis")
	}
...
func (a *App) seekToVehicle(next pullFunc, seekSeconds float64) error {
	...
	for {
		...
		if !ok {
			return fmt.Errorf("reached end of replay before a vehicle/engine appeared")
		}
...
func CaptureHapticsThroughSink(opts SinkCaptureOptions) (*SinkCapture, error) {
	...
	if opts.DurSeconds <= 0 {
		return nil, fmt.Errorf("DurSeconds must be > 0 for a real-time capture")
	}
